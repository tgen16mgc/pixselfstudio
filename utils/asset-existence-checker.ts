...
export async function checkAssetExists(assetPath: string): Promise<boolean> {
  ...
  try {
    ...
    if (isBaseAsset) {
      // Log for debugging
      console.log(`🔍 Base asset check: ${filename} -> EXISTS (from manifest)`)
    ...
    if (filename.includes('-') && filename.split('-').length > 2) {
      console.log(`🎨 Color variant check: ${filename}`)
      
      // Check if this is a color variant format: part-name-color.png
      // For simulation purposes in development, treat some common colors as existing
      // In production, you would check if the file actually exists on the server
      const lastPart = filename.split('-').pop()?.replace('.png', '') || '';
      
      if (['red', 'blue', 'green', 'black', 'brown', 'blonde'].includes(lastPart)) {
        console.log(`✨ SIMULATION: Color variant ${lastPart} exists: ${filename}`);
        assetExistenceCache.set(assetPath, true);
        return true;
      }
    }
    ...
    try {
      ...
      console.log(`${exists ? '✅' : '❌'} Asset ${exists ? 'exists' : 'does not exist'}: ${assetPath}`)
...
export async function getExistingColorVariants(
  ...
  console.log(`🔍 Checking color variants for ${baseAsset.id}`)
  ...
  // Reduced color sets for a more realistic check - only include colors that likely exist
  ...
  console.log(`🧐 Checking ${commonColors.length} possible colors for ${partType} asset: ${baseAsset.id}`)
  ...
  for (const colorName of commonColors) {
    ...
    // Format: /path/to/part-name-color.png
    const colorVariantPath = basePath.replace(filename, `${filenameWithoutExtension}-${colorName}.png`);
    
    console.log(`  🔎 Looking for variant: ${colorVariantPath}`);
    ...
    if (exists) {
      ...
      console.log(`  ✅ Found color variant: ${colorName} for ${baseAsset.id}`)
    } else {
      console.log(`  ❌ No ${colorName} variant for ${baseAsset.id}`)
    }
  }
  
  console.log(`🎯 Found ${existingVariants.length} color variants for ${baseAsset.id}:`, 
...
export function clearAssetCache(): void {
  ...
  console.log('🧹 Asset existence cache cleared')
}
